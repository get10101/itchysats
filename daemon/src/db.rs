...
/// Loads cfd that are not final yet
///
/// Finality can happen by:
/// 1. Recording the confirmation of a payout (spend) transaction on the blockchain
///     Cases: Collaborative settlement, CET, Refund
/// 2. The CFD failing early meaning it becomes irrelevant for processing
///     Cases: Setup failed, Taker's take order rejected
pub async fn load_non_final_cfd_ids(conn: &mut PoolConnection<Sqlite>) -> Result<Vec<OrderId>> {
    let ids = sqlx::query!(
        r#"
            select
                id as cfd_id,
...
mod tests {
    ...
    async fn given_collaborative_close_confirmed_then_do_not_load_non_final_cfd() {
        ...
        let cfd_ids = load_non_final_cfd_ids(&mut conn).await.unwrap();
