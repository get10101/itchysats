...
use daemon::model::cfd::{Cfd, Order, OrderId, Origin, UpdateCfdProposal, UpdateCfdProposals};
...
pub async fn start_both() -> (Maker, Taker) {
    ...
    let rollover_interval = time::Duration::seconds(1);
    let settlement_interval = time::Duration::seconds(24);
...
impl Maker {
    ...
    pub async fn get_update_cfd_proposal(
        ...
        tokio::time::timeout(tokio::time::Duration::from_secs(1), async {
            loop {
                let update_proposals = next(&mut self.update_proposals).await;
...
impl Taker {
    pub async fn start(
        ...
        let taker = daemon::TakerActorSystem::new(
            ...
            |_, _| oracle,
            |_, _| async { Ok(monitor) },
            AutoRolloverParams::new(rollover_offset, settlement_interval).unwrap(),
            N_PAYOUTS_FOR_TEST,
        )
        .await
        .unwrap();
