...
impl Actor {
    async fn handle(&mut self, msg: endpoint_monitor::ListenAddressesAdded) {
        if msg.added.contains(&self.listen_address) {
    ...
    async fn handle(
        &mut self,
        msg: endpoint_monitor::ListenAddressesRemoved,
        ctx: &mut xtra::Context<Self>,
    ) {
        if msg.removed.contains(&self.listen_address) {
